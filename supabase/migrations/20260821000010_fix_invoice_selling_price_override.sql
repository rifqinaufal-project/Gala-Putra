-- Use the selling price entered on the invoice when provided. Fall back to the
-- active customer price, then the product default price.
...
DECLARE
  ...
  selling_price NUMERIC; requested_selling_price NUMERIC; purchase_price NUMERIC; requested_purchase_price NUMERIC;
...
BEGIN
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    requested_selling_price := NULLIF((item->>'sellingPrice')::NUMERIC, 0);
    IF requested_selling_price IS NOT NULL THEN
      IF requested_selling_price <= 0 THEN RAISE EXCEPTION 'Harga jual invoice harus lebih dari nol'; END IF;
    ...
    IF actor_role IN ('OWNER','FINANCE') AND item ? 'purchasePrice' AND NULLIF(BTRIM(item->>'purchasePrice'), '') IS NOT NULL THEN
      requested_purchase_price := (item->>'purchasePrice')::NUMERIC;
      IF requested_purchase_price <= 0 THEN RAISE EXCEPTION 'Harga beli invoice harus lebih dari nol'; END IF;
      purchase_price := ROUND(requested_purchase_price, 2);
    ELSE
      SELECT pc.unit_cost INTO purchase_price
      FROM public.product_costs pc
      WHERE pc.product_id = product_row.id AND pc.effective_at <= NOW()
    ...
    INSERT INTO public.invoice_items(id, invoice_id, product_id, description_snapshot, quantity, margin_quantity,
      unit, selling_price_snapshot, purchase_price_snapshot, subtotal, product_cost_total, profit)
    VALUES (gen_random_uuid(), invoice_id, product_row.id,
      ...
      quantity_value, margin_value, product_row.default_unit, selling_price, purchase_price,
  ...
  UPDATE public.invoices SET subtotal = subtotal_value, discount = discount_value, total = total_value,
    ...
    transaction_margin = margin_percent, updated_at = NOW() WHERE id = invoice_id;
