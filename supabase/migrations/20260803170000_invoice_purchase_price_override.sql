-- Allow Owner/Finance to override HPP for one invoice only.
-- The product_costs master history is intentionally never modified here.
...
DECLARE
  ...
  requested_purchase_price NUMERIC;
  purchase_price_override_used BOOLEAN := FALSE;
...
BEGIN
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- Only Owner/Finance may provide an invoice-specific HPP override.
    -- Staff payloads are ignored and always use the active product HPP.
    IF actor_role IN ('OWNER'::public.user_role, 'FINANCE'::public.user_role)
      AND item ? 'purchasePrice'
      AND NULLIF(BTRIM(item->>'purchasePrice'), '') IS NOT NULL THEN
      requested_purchase_price := (item->>'purchasePrice')::NUMERIC;
      IF requested_purchase_price <= 0 THEN
        RAISE EXCEPTION 'Harga beli invoice harus lebih dari nol';
      END IF;
      purchase_price := ROUND(requested_purchase_price, 2);
      purchase_price_override_used := TRUE;
    ELSE
    ...
    VALUES (gen_random_uuid(), invoice_id, product_row.id,
      ...
      quantity_value, product_row.default_unit, selling_price, purchase_price,
  ...
  VALUES (auth.uid(), COALESCE(actor_name, 'User'), 'invoices', invoice_id, 'INVOICE_CREATED',
    jsonb_build_object('status', requested_status, 'total', total_value, 'invoice_number', invoice_number_value,
      'purchase_price_override_used', purchase_price_override_used));
