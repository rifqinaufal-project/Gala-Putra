-- Allow each load to carry the selling price agreed with its destination.
-- This is a snapshot and does not overwrite the product master price.
ALTER TABLE public.load_items
  ADD COLUMN IF NOT EXISTS selling_price_per_kg NUMERIC(14, 2)
  CHECK (selling_price_per_kg IS NULL OR selling_price_per_kg > 0);
...
BEGIN
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    row_selling_price := ROUND(COALESCE((item->>'sellingPricePerKg')::NUMERIC, 0), 2);
    IF row_kg <= 0 OR row_price <= 0 THEN
      RAISE EXCEPTION 'Berat dan harga beli per kg harus lebih dari nol';
    END IF;
    IF row_selling_price <= 0 THEN
      RAISE EXCEPTION 'Harga jual pabrik per kg harus lebih dari nol';
    END IF;
    IF NOT EXISTS (
  ...
  ON CONFLICT (period_key) DO UPDATE
    SET last_value = invoice_sequence.last_value + 1, updated_at = NOW()
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    row_selling_price := ROUND((item->>'sellingPricePerKg')::NUMERIC, 2);
    ...
    INSERT INTO public.load_items(
      ...
      row_price, row_selling_price, ROUND(row_kg * row_price, 2),
  ...
  LOOP
    ...
    VALUES (v_period_key, 1)
    ON CONFLICT (period_key) DO UPDATE
      SET last_value = bill_sequence.last_value + 1, updated_at = NOW()
  ...
  FOR source_row IN
    SELECT li.product_id, li.product_name_snapshot,
      ...
      MAX(li.selling_price_per_kg) AS entered_selling_price
    FROM public.load_items AS li
    WHERE li.load_id = load_id_value
    GROUP BY li.product_id, li.product_name_snapshot
  LOOP
    SELECT COALESCE(
      source_row.entered_selling_price,
      (
    ...
    IF selling_price IS NULL OR selling_price <= 0 THEN
      RAISE EXCEPTION 'Harga jual pabrik untuk % belum tersedia', source_row.product_name_snapshot;
    END IF;
