-- Fix PostgreSQL 42702: PL/pgSQL variable period_key conflicted with
-- period_key columns in the invoice and supplier-bill sequence tables.

CREATE OR REPLACE FUNCTION public.next_gala_load_number(p_date DATE)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_period_key TEXT := TO_CHAR(p_date, 'YYYY/MM');
  v_sequence_value INTEGER;
BEGIN
  INSERT INTO public.load_number_sequences AS load_sequence (period_key, last_value)
  VALUES (v_period_key, 1)
  ON CONFLICT (period_key) DO UPDATE
    SET last_value = load_sequence.last_value + 1,
        updated_at = NOW()
  RETURNING load_sequence.last_value INTO v_sequence_value;

  RETURN 'MUA/' || v_period_key || '/' || LPAD(v_sequence_value::TEXT, 4, '0');
END;
$$;
...
BEGIN
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    purchase_total := purchase_total + row_total;
  END LOOP;
  ...
  INSERT INTO public.loads(
    ...
    'DISPATCHED', total_kg, purchase_total, load_cost_total,
  ...
  FOR source_row IN
    SELECT source_id, ROUND(SUM(purchase_total), 2) AS total
    FROM public.load_items
    WHERE load_id = load_id_value
    GROUP BY source_id
  LOOP
  ...
  FOR source_row IN
    SELECT product_id, product_name_snapshot,
      ROUND(SUM(quantity_kg), 3) AS quantity_kg,
      ROUND(SUM(purchase_total) / NULLIF(SUM(quantity_kg), 0), 2) AS cost
    FROM public.load_items
    WHERE load_id = load_id_value
  ...
  SET subtotal = subtotal_value,
      ...
      total_product_cost = (
        SELECT ROUND(COALESCE(SUM(product_cost_total), 0), 2)
      ...
      transaction_margin = CASE
        ...
        ELSE ROUND(((
          subtotal_value - (
            SELECT COALESCE(SUM(product_cost_total), 0)
            FROM public.invoice_items
            WHERE invoice_id = invoice_id_value
          ) - load_cost_total
